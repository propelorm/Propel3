    ...
    public function process()
    {
        ...
        $this->getDefinition()
            ->setConstant(PhpConstant::create(
                'LEFT_COL',
                $entityName . '.' . $this->getBehavior()->getFieldForParameter('left_field')->getName()
            )
                ->setDescription('Left field for the set'))
            ->setConstant(PhpConstant::create(
                'RIGHT_COL',
                $entityName . '.' . $this->getBehavior()->getFieldForParameter('right_field')->getName()
            )
                ->setDescription('Right field for the set'))
            ->setConstant(PhpConstant::create(
                'LEVEL_COL',
                $entityName . '.' . $this->getBehavior()->getFieldForParameter('level_field')->getName()
            )
        ...
        if ($this->getBehavior()->useScope()) {
            $this->getDefinition()
                ->setConstant(PhpConstant::create(
                    'SCOPE_COL',
                    $entityName . '.' . $this->getBehavior()->getFieldForParameter('scope_field')->getName()
                )
